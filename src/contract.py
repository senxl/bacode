...
def get_symbol_filters(client, symbol: str) -> dict:
    ...
    price_f = next(f for f in symbol_info["filters"] if f["filterType"] == "PRICE_FILTER")
    lot_f = next(f for f in symbol_info["filters"] if f["filterType"] == "LOT_SIZE")
    notional_f = next(f for f in symbol_info["filters"] if f["filterType"] == "MIN_NOTIONAL")
    return {
        "tick_size": float(price_f["tickSize"]),
        "step_size": float(lot_f["stepSize"]),
        "min_notional": float(notional_f["notional"]),
    }
